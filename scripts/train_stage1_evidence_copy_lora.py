#!/usr/bin/env python
"""Smoke LoRA training for Stage 1 evidence-copy curriculum only."""
...
DEFAULT_MODEL = PROJECT_ROOT / ".models" / "modelscope" / "Qwen" / "Qwen3-0___6B"
DEFAULT_OUTPUT = (
    PROJECT_ROOT
    / "training"
    / "checkpoints"
...
DEFAULT_REPORT = PROJECT_ROOT / "training" / "reports" / "stage1_evidence_copy_train.json"
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Train Stage 1 evidence-copy smoke LoRA")
    ...
    protected = PROJECT_ROOT / "training" / "checkpoints" / "qwen3-0.6b-medlearn-lora" / "final"
    if args.output_dir.resolve() == protected.resolve():
    ...
    tokenizer = AutoTokenizer.from_pretrained(str(args.model), use_fast=True, local_files_only=True)
    ...
    report = {
        ...
        "protected_paths_not_overwritten": [
            str(PROJECT_ROOT / "training" / "checkpoints" / "qwen3-0.6b-medlearn-lora" / "final"),
        ],
    }
