...
def extract_section_name(title: str) -> str:
...
def generate_catalog_from_toc(toc: list[dict[str, Any]], book_name: str) -> dict[str, Any]:
    ...
    for entry in toc:
        ...
        if not title or title in ['封面页', '书名页', '版权页', '编委名单', '目录', '前言', '序言', '新形态教材使用说明']:
            continue
        ...
        if level == 1 and '章' in title:
            ...
            section_name = extract_section_name(title)
            current_section = {
                'name': section_name,
                'label': title,
                'pageNum': page,
...
def main():
    ...
    for book_dir in generated_dir.iterdir():
        ...
        try:
            ...
            total_sections = sum(len(ch.get('children', [])) for ch in chapters)
            print(f'[ok] {catalog_name}: {len(chapters)} chapters, {total_sections} sections')
