#!/usr/bin/env python
"""Smoke LoRA training for Stage 2 evidence + content distill (continues from Stage 1)."""
...
DEFAULT_REPORT = PROJECT_ROOT / "training" / "reports" / "stage2_evidence_content_train.json"
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Train Stage 2 evidence+content smoke LoRA")
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.65,
        help="Fraction of max_length reserved for assistant completion (stage2 JSON is longer).",
    )
    ...
    if not args.base_adapter.exists():
        raise RuntimeError(f"Stage 1 adapter not found: {args.base_adapter}")
    ...
    if trainable == 0:
        raise RuntimeError("No trainable LoRA parameters found after loading Stage 1 adapter")
    ...
    report = {
        "stage": "stage2_evidence_content_distill",
