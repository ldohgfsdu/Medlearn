...
Parity standard (pinned per user spec, 2026-07-03):
  ...
  6. Divergence has explicit cause classification: when cross-parser
     anchor IDs differ, the test documents the known cause (font/size/
     color/flags absent in Docling vs present in PyMuPDF, cell-level
     vs span-level segmentation, etc.).
...
def assert_canonical_text_coverage(
    ...
    """Check 2: each parser extracts non-trivial text from the same page."""
...
def assert_divergence_has_explicit_cause(
    ...
    - Docling adds docling_confidence/docling_from_ocr provenance fields
    ...
    if len(docling_page.raw_spans) > 0:
        first_docling_span = docling_page.raw_spans[0].spans[0]
        test.assertEqual(
            first_docling_span.get("font"), "",
            "Docling divergence cause: font is sentinel empty string",
        )
        test.assertEqual(
            first_docling_span.get("size"), 0.0,
            "Docling divergence cause: size is sentinel 0.0",
        )
        test.assertIn(
            "docling_confidence", first_docling_span,
            "Docling divergence cause: docling_confidence field present",
        )

    # Known cause 2: span count likely differs (cell-level vs line-level)
    pymupdf_count = len(pymupdf_page.raw_spans)
    docling_count = len(docling_page.raw_spans)
    test.assertNotEqual(
        pymupdf_count, docling_count,
        f"Expected span count divergence (cell-level vs line-level); "
        f"got pymupdf={pymupdf_count}, docling={docling_count}",
    )
...
class CrossParserDivergenceTests(unittest.TestCase):
    """Check 6: cross-parser divergence has explicit cause classification."""
    ...
    def test_page_identity_consistent_across_parsers(self):
        assert_page_identity_consistency(
            self,
            [self.pymupdf_snap, self.docling_snap],
