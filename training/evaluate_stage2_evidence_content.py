#!/usr/bin/env python
"""Evaluate Stage 2 evidence + content distill LoRA adapters."""
...
from stage1_eval_metrics import row_to_stage1_case  # noqa: E402
from stage2_eval_metrics import (  # noqa: E402
    build_stage2_markdown_report,
    evaluate_stage2_case,
    evaluate_stage2_gates,
    summarize_stage2_cases,
)
...
REPORT_JSON = TRAINING_DIR / "reports" / "stage2_evidence_content_eval.json"
REPORT_MD = TRAINING_DIR / "reports" / "stage2_evidence_content_eval.md"
FAILURE_JSONL = TRAINING_DIR / "reports" / "stage2_evidence_content_failures.jsonl"
...
def golden_to_stage2_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    converted: list[dict[str, Any]] = []
    for row in rows:
        parsed = parse_train_sample(row)
        built = build_curriculum_row(parsed, stage="stage2_evidence_content_distill", stage_index=2)
        if built:
            built["id"] = f"{row.get('id') or 'golden'}::stage2_evidence_content_distill"
            converted.append(built)
    return converted


def row_to_stage2_case(row: dict[str, Any], suite: str) -> EvalCaseInput:
    return row_to_stage1_case(row, suite)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Evaluate Stage 2 evidence+content adapter")
    ...
    golden_stage2_rows = golden_to_stage2_rows(load_jsonl(args.golden_eval))

    suites: dict[str, list[EvalCaseInput]] = {
        "validation_holdout": [row_to_stage2_case(row, "validation_holdout") for row in holdout_rows],
        "golden_stage2": [row_to_stage2_case(row, "golden_stage2") for row in golden_stage2_rows],
    }
    ...
    case_results = [evaluate_stage2_case(case) for case in generated]
    ...
    for suite_name, suite_cases in suites.items():
        ...
        suite_reports[suite_name] = {
            "aggregate": summarize_stage2_cases(suite_results),
            "cases": suite_results,
        }

    aggregate = summarize_stage2_cases(case_results)
    gates = evaluate_stage2_gates(aggregate)
    report = {
        "meta": {
            "evaluated_at": utc_now_iso(),
            "mode": "evidence_content_distill",
    ...
    args.report_md.write_text(build_stage2_markdown_report(report), encoding="utf-8")
